...
runuser -u "$APP_USER" -- env RESET_ADMIN_LIST=1 \
  bash -lc "set -a; source '$ENV_FILE'; set +a; cd '$APP_DIR'; /usr/local/bin/npm run reset-admin-password"
printf '\n'
...
printf '%s' "$ADMIN_PASSWORD" | runuser -u "$APP_USER" -- env RESET_ADMIN_USERNAME="$ADMIN_USERNAME" \
  bash -lc "set -a; source '$ENV_FILE'; set +a; cd '$APP_DIR'; /usr/local/bin/npm run reset-admin-password"
