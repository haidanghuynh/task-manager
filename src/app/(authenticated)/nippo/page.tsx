      ...
      {mode === "mine" ? (
        loading ? <p className="py-12 text-center text-gray-500">Loading...</p> : myEmployee && (
          <div className="space-y-5">
            ...
            <section className="rounded-xl border bg-white p-5"><h3 className="mb-3 font-semibold text-gray-900">{text.candidate}</h3>{candidateTasks.length === 0 ? <p className="text-sm text-gray-500">{text.noTask}</p> : <div className="grid gap-2 md:grid-cols-2">{candidateTasks.map((task) => <div key={task.id} className="flex items-center gap-3 rounded-lg border p-3"><span className="h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: task.workType === "DAILY" ? dailyWorkColor(task.dailyCategory, dailyCategories) : task.product?.color || "#6B7280" }} /><div className="min-w-0 flex-1"><Link href={`/tasks/${task.id}`} className="block truncate text-sm font-medium text-blue-700 hover:underline">{task.taskCode}: {task.taskName}</Link><p className="text-xs text-gray-500">{task.workType === "DAILY" ? dailyWorkLabel(task.dailyCategory, lang, dailyCategories) : task.product?.name} · {task.previousProgress}%</p></div><button disabled={formLocked || addedTaskIds.has(task.id)} onClick={() => addTask(task)} className="rounded border px-2 py-1 text-xs text-blue-700 disabled:text-gray-400">{addedTaskIds.has(task.id) ? text.added : text.add}</button></div>)}</div>}</section>
